...
<div class="polling yellow">
	...
	//display options with radio buttons
			...
			if($query){
				...
				foreach($query->result() as $row){
					echo '<label for="option-'.$row->id_pilihan_polls.'" ><input type="radio" name="poll" value="'.$row->id_pilihan_polls.'" id="option-'.$row->id_pilihan_polls.'" /> 
					'.$row->judul_pilihan_polls.'</label>';
				}
				echo '<p><input type="submit"  value="Submit" class="btn" /></p></form>';
				echo '<p><a href="'.base_url().'modul/polling_sidebar?result=1" id="viewresult">View result</a></p></div>';
			}
		}
	}
	else{
